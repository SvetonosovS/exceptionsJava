...
public class UserInputConsoleReader extends UserInputReader {
  ...
  public UserData readUserData() throws UserDataException {
    while (true) {
      System.out.println("Введите данные через пробел, в следующем порядке: Фамилия Имя Отчество Дата рождения (dd.mm.yyyy) Номер телефона Пол (f или m)");
      ...
      try {
        ...
        if (data.length != 6) {
          throw new UserDataException("Ввели меньше данных, чем требуется");
        }
